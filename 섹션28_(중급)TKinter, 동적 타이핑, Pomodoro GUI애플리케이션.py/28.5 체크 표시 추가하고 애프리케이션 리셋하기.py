...
def reset_timer():
    ...
    # 리셋하고 타이머가 00:00이 되게해야한다. 상단 라벨도 timer가 되도록 설정하고 체크 표시도 제거
...
def start_timer():
    ...
    # #만약 카운트다운이 첫번째,3번째,5번째,7번재 반복이라면
    # count_down(work_sec) # 작업초를 기준으로 25분까지 카운트다운 하지만
    # # 만일 여덟 번째 반복이라면 우리의 긴 휴식초까지 카운트다운 하게되고
    # count_down(long_break_sec)
    # #하지만 만일 두번째,4번째, 또는 6번째 반복이라면 우리의 짧은ㅇ 휴식 초까지 카운트 다운 하게 할수 있을까
